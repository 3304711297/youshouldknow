...
1. **IRQL = 255 (`HIGH_LEVEL`)**：
   这是 Windows 内核体系中的最高硬件中断级别（用于 APIC Timer 硬件时钟中断、跨核调度）。在该级别下，系统**严禁发生任何缺页调度（Page Fault）**，一旦寻址失败，内核无法调度虚拟内存管理器自愈，只能立即硬性蓝屏刹车。
...
### 2. 为什么关闭 C-State 后依然会在 `IRQL=255` 崩溃？
...
* 一旦系统出现突发的多进程并发切换或网络硬件中断，CPU 在响应最高时钟中断（`HIGH_LEVEL`）的纳秒瞬间，供电电感产生微小的瞬态压降（Transient Vdroop），Ring Bus 内部寻址总线产生单比特翻转（Bit-flip），内存指针损坏为无效地址，硬性诱发 `0x0000000A`。
