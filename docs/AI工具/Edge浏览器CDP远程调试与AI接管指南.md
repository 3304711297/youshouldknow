...
## 二、为什么不能用命令行参数开调试（重要前置认知）

传统做法是给浏览器加启动参数 `--remote-debugging-port=9222`。但从 **Chromium 136 起，使用默认用户数据目录时该参数会被直接忽略**（防止 Cookie 被外部程序窃取的安全加固），且：

- 显式传 `--user-data-dir` 指向同一个默认目录：**同样无效**；
- 注册表策略 `RemoteDebuggingAllowed=1`：**无效**；
- junction/符号链接改路径伪装成非默认目录：端口能通，但会触发 Profile 完整性保护，**曾导致全部扩展被注销清空**，强烈不建议。

正确路径是微软官方提供的浏览器内开关（下一节）。
