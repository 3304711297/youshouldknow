...
- **验证机制（“四灯”完整验证法，避免 Enabled 假象）**：
  - **第一灯（驱动面板）**：NVIDIA 控制面板「系统信息」中 `可重设大小的基底位` 显示为 `是`；AMD 驱动中心「显存直通达 / SAM」显示为开启；
  - **第二灯（GPU-Z 状态）**：GPU-Z 主界面右下角 `Resizable BAR` 参数列显示为 `Enabled`；
  - **第三灯（窗口大小拉满，最关键排查点）**：切到 GPU-Z「Advanced」标签页选择 `PCIe Resizable BAR`（或在设备管理器显卡属性中查看资源内存范围 `Memory Range`）——**显存窗口大小必须接近物理显存容量**（例如 8GB 显卡约为 8192MB，16GB 显卡约为 16384MB）。若出现 **`Enabled + 256 MB`**，说明系统能力位虽开，但底层显存大窗并未真正拉满（常见于驱动过旧、VBIOS 未激活大窗口或看成了核显节点）；
  - **第四灯（游戏实测对照）**：对比开启前后的 1% Low 帧。
...
- **Intel Arc（锐炫）显卡强依赖（必开项）**：
  - 区别于 N/A 卡的锦上添花，**Intel Arc 架构对 ReBAR 呈强依赖**。若平台未开启 ReBAR，Intel 锐炫显卡在多款现代 3A 游戏中性能会直接暴跌 **20% 以上**；
- 整机功耗增加通常不超过 3%~5%。

## 翻车恢复与主板排坑

- **技嘉平台 Auto 陷阱**：技嘉 BIOS 中 `Above 4G Decoding` 与 `Re-Size BAR Support` 拆分为两个子项，**必须手动全部设为 Enabled**，不可保留在 Auto（部分版本 Auto 判定存在唤醒失效 BUG）；
- **老板“挖矿”别名**：部分老旧 BIOS（如早期微星）中，`Above 4G Decoding` 曾被命名为 `挖矿 (Mining)`，功能完全一致；
...
- [电脑 BIOS 选项全科普 EP17/Resizable BAR【暮里学姐】（重制版，BV1drY86aEzY）](https://www.bilibili.com/video/BV1drY86aEzY/)
- [电脑 BIOS 选项全科普 EP07/Above 4G【暮里学姐】（旧版，BV1LFEt6QE4e，已由重制版 EP16 替代）](https://www.bilibili.com/video/BV1LFEt6QE4e/)
- [电脑 BIOS 选项全科普 EP08/Resizable BAR【暮里学姐】（旧版，BV11tEu6SEKX，已由重制版 EP17 替代）](https://www.bilibili.com/video/BV11tEu6SEKX/)
