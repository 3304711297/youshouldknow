...
### 4.5 网卡高级硬件属性（中断节流、大包发送卸载与缓冲区）的工程权衡

社区部分“网络深度优化器”（如 ALit-NetworkOptimizer 等）常将网卡设备管理器的高级属性全部拉满或盲目关闭，这类做法往往存在严重的系统级工程代价：

1. **中断节流（Interrupt Moderation / `*InterruptModeration`）**：
   - **机制**：网卡默认开启中断节流，将微秒内的多个数据包聚合为单次硬件中断，再由系统调度单次 ISR/DPC 处理，保护 CPU 免受中断风暴侵扰；
   - **代价与权衡**：关闭中断节流（设为 `0`）确实能在极少数轻载极限竞技中减少几个微秒的排队延迟，但代价是硬件中断频率呈指数级飙升。在多核、高吞吐或笔记本平台上，过高的 DPC/ISR 处理开销会直接挤占游戏主线程的 CPU 时间片，极易引发游戏帧时间突发尖峰（Microstuttering）。必须通过 LatencyMon 实测 DPC 延迟，绝不能当作“无脑通用优化”；
2. **大包发送卸载（Large Send Offload / `*LSOv2IPv4` / `*LSOv2IPv6`）**：
   - **机制**：LSO 允许 TCP 协议栈将最大 64KB 的上层大缓冲直接交给网卡芯片，由网卡硬件自主分片（Segmentation）并计算校验和，大幅降低 CPU 负载；
   - **代价与权衡**：针对高频小包互动的 FPS 游戏，小包本身不触发分段，关 LSO 影响甚微；但一旦盲目全局关闭 LSO，在进行后台下载、Steam 游戏更新或局域网文件传输时，CPU 将被强制拉回处理每一个 1500 字节的 TCP 包，导致传输吞吐骤降且 CPU 占用剧增；
3. **收发缓冲区大小（Receive/Transmit Buffers）与“伪回滚”陷阱**：
   - 部分优化器将环形缓冲区强制改写为最大值（如 2048），并在所谓的“一键还原”中硬编码恢复为固定值（如 512）；
   - **严重缺陷**：不同网卡厂商的默认队列长度完全不同（例如部分网卡出厂默认就是 1024）。硬编码的“恢复”本质上是又一次非受控篡改。任何驱动级高级属性修改，必须遵循严格的首次状态快照精准回滚原则。
