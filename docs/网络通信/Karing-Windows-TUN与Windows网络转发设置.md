...
| Karing 客户端的具体行为与规则模式说明 | ⚠️（2026-08-29 重核：核查时点版本 v1.2.24.2707（2026-08-28 发布；版本演进较快，以 GitHub Releases 最新版为准），GitHub 预发布渠道活跃；官方文档确认 TUN 为自带独立虚拟网卡实现，Windows 端需管理员权限并自动安装驱动。正文“自带 TUN 不等于依赖系统 NDIS 枚举器，可禁用 Forwarding 但无法确认时不要盲禁”的结论仍成立。行为细节仍随上游版本变化，karing.app/faq 已复核；GitHub 仓库内 faq.md 页面当日网络超时未能直接打开，以 karing.app/faq 为准） |
