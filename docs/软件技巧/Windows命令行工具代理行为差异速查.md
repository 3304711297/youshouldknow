...
上表解决的是"怎么让工具走代理"，反向需求同样高频：**某些目标必须绕过代理**。这靠 `NO_PROXY` 白名单实现，详见
[《本地代理环境的 NO_PROXY 白名单与程序库代理行为》](./本地代理环境的NO_PROXY白名单与程序库代理行为.md)。

这里只记三条最容易踩的：

| 陷阱 | 说明 |
| --- | --- |
| 本机环回也会被代理 | 只设 `ALL_PROXY` 而不设 `NO_PROXY`，`127.0.0.1` 的请求照样被送进代理；代理软件"开着却没连节点"时，本机服务互调会全部超时 |
| `pip` 要填**两个**域名 | `pypi.org`（索引）+ `files.pythonhosted.org`（CDN 下载）。只填前者会出现"能查到包、下载全部失败"的半通假象 |
| 端口在监听 ≠ 代理可用 | 代理客户端未连节点时，本地端口**仍然 LISTENING**，`curl -x` 会挂满超时周期才失败，无法区分"代理没起"与"节点不通" |

> ⚠️ 另需注意：上表覆盖的是**命令行命令**，而程序**内部使用的 HTTP 库**（Rust `reqwest`、Python `httpx`、Node `undici` 等）行为与之并不一致，同环境下可能完全相反。这层差异见上文链接的专文。
